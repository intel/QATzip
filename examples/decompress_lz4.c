...
static int deCompressFile(int in_file, int out_file)
{
    ...
    if (rc != QZ_OK) {
        fprintf(stderr, "qzGetDefaultsLZ4 failed\n");
        return -1;
    }
    ...
    dst_buffer_size = src_buffer_size * 10;
    ...
    int ret = qzDecompress(&session, src_buffer, (unsigned int *)(&bytes_read), dst_buffer, &dst_buffer_size);
    assert(QZ_OK == ret);
...
static char* createOutFilename(const char* filename)
{
    ...
    strncpy(outSpace, filename, outL);
    printf("%s\n", outSpace);
...
int main(int argc, const char** argv)
{
    ...
    deCompressFile(in_file, out_file);
    close(in_file);
    close(out_file);
    free(outFilename);
    return 0;
